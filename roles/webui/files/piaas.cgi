...
if [ ! -z "$QUERY_STRING" ]; then
  ...
  case $ACTION in
    ...
    ;;
    unassigned)
    diff <(ls /data/tftpboot/) <(ls /etc/piaas/nodes/) | grep -e '^>' | cut -d " " -f2
    ;;
  esac
fi
...
echo '<h1>Instances</h1>'
 echo '<table> <tr> <th>Name</th> <th>Image</th> <th>Serial</th> <th>Power</th> <th>Ping</th> <th>SSH</th> <th>VNC</th> <th>Action</th> <th></th> </tr>'
  ...
  do echo '<tr>'
    ...
    echo '</td>'
    echo '<td>'
      image=$(grep " /data/nfs/$instance " /etc/fstab | grep -oP "(?:^|,)lowerdir=\K[^,]+" | xargs basename)
      echo "$image"
    ...
    then
      ...
      nodes=$(diff <(ls /data/tftpboot/) <(ls /etc/piaas/nodes/) | grep -e '^>' | cut -d " " -f2)
      for node in $nodes;
      do
        source /etc/piaas/nodes/$node
